...
module Sentry
  class ProcessRunner
    ...
    @logger : Maze::Environment::Logger
    ...
    private def build_app_process
      ...
      Maze::CLI::Helpers.run(@build_command)
    end

    private def create_watch_process
      log "Starting #{project_name}..."
      Maze::CLI::Helpers.run(@run_command, wait: false, shell: false)
    end

    private def create_npm_process
      node_log "Installing dependencies..."
      Maze::CLI::Helpers.run("npm install --loglevel=error && npm run watch", wait: false)
