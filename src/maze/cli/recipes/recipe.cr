  ...
  class Recipe
    ...
    def generate(template : String, options = nil)
      ...
      when "scaffold"
        ...
        if model == "crecto"
          Maze::CLI::CrectoMigration.new(name, @fields).render(directory, list: true, color: true)
        else
          Maze::CLI::GraniteMigration.new(name, @fields).render(directory, list: true, color: true)
        end
        Model.new(name, @recipe, @fields).render(directory, list: true, color: true)
...
class Teeplate::RenderingEntry
  ...
  def list(s, color)
    Maze::CLI.logger.info s.colorize.fore(color).to_s + local_path, "Generate", :light_cyan
  end
end
