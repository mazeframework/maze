...
module Maze::CLI
  extend Helpers
  describe "maze routes" do
    begin
      ENV["CRYSTAL_CLI_ENV"] = "test"
      context "in an `maze new` app with default options" do
        cleanup
        ...
        describe "with the default routes" do
          ...
          it "outputs the static file handler" do
            expected = "Maze::Controller::Static"
            output.should contain expected
            line = output_lines.find("") { |line| line.includes? expected }
            expectations = %w(get Maze::Controller::Static index static /*)
