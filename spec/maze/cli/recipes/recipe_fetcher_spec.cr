...
module Maze::Recipes
  describe RecipeFetcher do
    ...
    context "using the recipe cache in the current directory" do
      Spec.before_each do
        Dir.mkdir_p("#{MAZE_RECIPE_FOLDER}/mydefault/app")
        Dir.mkdir_p("#{MAZE_RECIPE_FOLDER}/mydefault/controller")
        Dir.mkdir_p("#{MAZE_RECIPE_FOLDER}/mydefault/scaffold")
      end

      Spec.after_each do
        FileUtils.rm_rf("#{MAZE_RECIPE_FOLDER}/mydefault")
      end
